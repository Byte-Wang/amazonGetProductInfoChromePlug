...
function getMainContentView(asin = "") {
    ...
    const contentView = "<div class=\"feixun_plug_row feixun_plug_bottom_border\">\
                                        ...
                                        <input type=\"button\" id=\"favorite_button"+idSubfix+"\" value=\"收藏\"/>\
                                        </div>\
                                    </div>\
...
function parserToTitleFeatureDiv(retryTimes){
    ...
    const favoriteBtn = document.getElementById('favorite_button');

    const asin = getASIN();
    const stationId = getStationId(getRegion());
    if (window.localStorage.getItem("favorite_"+asin+stationId) == 1) {
        favoriteBtn.setAttribute('value',"取消收藏");
    } else {
        favoriteBtn.setAttribute('value',"收藏");
    }
    ...
    favoriteBtn.addEventListener('click', function() {
        chrome.storage.sync.get('userInfo', function(result) {
            ...
            if (!userInfo || !userInfo.token) {
                FXLog("[test] 没有token，未登录：");
                callback({code: 401, desc: "请先登录！"});
                feixunShowToast('favorite_button','请先登录！');
                return;
            }
            let url = 'http://119.91.217.3:8087/index.php/admin/index/setFavoritePlugProduct';
            const asin = getASIN();
            const stationId = getStationId(getRegion());
            let favoriteValue = 1;
            if (window.localStorage.getItem("favorite_"+asin+stationId) == 1) {
            ...
            chrome.runtime.sendMessage({
                ...
                FXLog("[test] setFavoritePlugProduct:",response);
                if (response && response.data && response.data.code == 200) {

                    window.localStorage.setItem("favorite_"+asin+stationId, favoriteValue);
                    ...
                    feixunShowToast('favorite_button',favoriteValue ? '收藏成功' : '取消收藏成功');
                } else {
                    feixunShowToast('favorite_button','操作失败');
                }
    ...
    divs.forEach(div => {  
        ...
        if (div.hasAttribute('data-asin') && div.getAttribute('data-asin').trim() !== '') {  
            ...
            if (declarativeSpan) {  
                ...
                const favoriteBtn = document.getElementById('favorite_button_'+asin);
                const stationId = getStationId(getRegion());
                if (window.localStorage.getItem("favorite_"+asin+stationId) == 1) {
                    favoriteBtn.setAttribute('value',"取消收藏");
                } else {
                ...
                favoriteBtn.addEventListener('click', function() {
                    chrome.storage.sync.get('userInfo', function(result) {
                        ...
                        if (!userInfo || !userInfo.token) {
                            FXLog("[test] 没有token，未登录：");
                            callback({code: 401, desc: "请先登录！"});
                            feixunShowToast('favorite_button_'+asin,'请先登录！');
                            return;
                        }
                        let url = 'http://119.91.217.3:8087/index.php/admin/index/setFavoritePlugProduct';
                        const stationId = getStationId(getRegion());
                        ...
                        chrome.runtime.sendMessage({
                            ...
                            if (response && response.data && response.data.code == 200) {
                                ...
                                feixunShowToast('favorite_button_'+asin,favoriteValue ? '收藏成功' : '取消收藏成功');
                            } else {
                                feixunShowToast('favorite_button_'+asin,'操作失败');
                            }
